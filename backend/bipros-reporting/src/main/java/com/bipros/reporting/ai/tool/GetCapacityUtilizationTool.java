...
public class GetCapacityUtilizationTool implements Tool {
    ...
    public String description() {
        return "Capacity EFFICIENCY view (output vs productivity norm per resource-day) — "
                ...
                + "Returns Manpower + Equipment sections, each with per-role rows for Day / "
                + "Month / Cumulative buckets. Every row's bucket carries: qty (the unit-of-work "
                + "the role actually executed — e.g. 10 Cum, 4 nos), budget_days, planned_nos, "
                + "actual_days, actual_nos, actual_days_untracked (days on activities with no "
                + "norm), utilization_pct, cost_implication, plus a norm_source ∈ "
