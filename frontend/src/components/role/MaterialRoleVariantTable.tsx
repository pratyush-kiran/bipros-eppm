...
import {
  ...
  type MaterialRoleVariantRequest,
...
const UNITS = ["MT", "Bag", "Cum", "Litre", "Nos", "Kg"] as const;

export function MaterialRoleVariantTable({ roleId }: Props) {
  const qc = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<MaterialRoleVariantRequest>({
    specGrade: "",
    unit: "MT",
    rate: 0,
    active: true,
  ...
  const { data: variantsResp } = useQuery({
    queryKey: ["material-role-variants", roleId],
  ...
  const create = useMutation({
    mutationFn: (req: MaterialRoleVariantRequest) =>
      roleRateApi.createMaterialVariant(roleId, req),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["material-role-variants", roleId] });
      setShowForm(false);
      setForm({ specGrade: "", unit: "MT", rate: 0, active: true });
      setError(null);
    },
    onError: (e: unknown) =>
  ...
  const remove = useMutation({
    mutationFn: (id: string) => roleRateApi.deleteMaterialVariant(id),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["material-role-variants", roleId] }),
  });
      ...
      {showForm && (
        <div className="rounded-md border border-neutral-700 p-4">
          ...
          <div className="mt-3 flex gap-2">
            <button
              disabled={!form.specGrade || !form.rate || create.isPending}
