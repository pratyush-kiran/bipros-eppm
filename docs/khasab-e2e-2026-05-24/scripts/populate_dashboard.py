...
# Get the issue table columns first
cols = sql("SELECT column_name FROM information_schema.columns WHERE table_schema='project' AND table_name='dpr_issues' ORDER BY ordinal_position")
print(f"  dpr_issues columns: {[c.split('|')[0] for c in cols]}")
